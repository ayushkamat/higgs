...
class NeuralNetwork(ABC):
    ...
    def test_kaggle(self, dataset: Dataset, name: str = '') -> Dict[str, List[np.ndarray]]:
        ...
        filename = 'kaggle_predictions'
        if name:
            filename += '-' + name
        filename += '.csv'
        kaggle.to_csv(self.logger.save_dir + filename, index=False)
        return kaggle
